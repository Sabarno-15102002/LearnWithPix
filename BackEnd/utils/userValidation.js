...
// const age = Joi.number().pattern(/^[0-9]/).required().messages({
//     'number.pattern.base': `You age should not contain anything other than numbers`,
//     'number.empty': `Age cannot be an empty field`
// });

const age = Joi.number().integer().required().min(0).max(200).messages({
    'number.empty': `Age cannot be an empty field`
})

exports.loginSchema = Joi.object({ email, password});

exports.registerSchema = Joi.object({ name, email, password,age});